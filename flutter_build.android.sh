...
if [ ! -d "./build_flutter_aarch64" ]; then
    sed "s|EMSDK:|$1|g" ./cross/flutter_android_aarch64.txt > /tmp/.flutter_cross.txt
...
if [ ! -d "./build_flutter_x86_64" ]; then
    sed "s|EMSDK:|$1|g" ./cross/flutter_android_x86_64.txt > /tmp/.flutter_cross.txt
...
ls -lrt build_flutter_aarch64/src/bindings/flutter/libthorvg.so
ls -lrt build_flutter_x86_64/src/bindings/flutter/libthorvg.so
